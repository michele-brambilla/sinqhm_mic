...
void process_0mq_init_daq(void);
void process_0mq(packet_type *p);
void process_0mq_mb(packet_type *p);
void process_0mq_destruct(void);
...
int process_0mq_construct(void)
{
  ...
  if(!histo_descr_ptr)
  {
    dbg_printf(DBGMSG_ERROR, "process_0mq_construct(): can not get data pointer\n");
  ...
  if(!histoDataPtr)
  {
    dbg_printf(DBGMSG_ERROR, "process_0mq_construct(): can not get histo data pointer\n");
    return SINQHM_ERR_NO_HISTO_DATA_PTR;
  }

  process_packet_fcn   = process_0mq;
  process_init_daq_fcn = process_0mq_init_daq;
  process_destruct_fcn = process_0mq_destruct;
  nbank = histo_descr_ptr->nBank;

  if (nbank == 1)
  {
    dbg_printf(DBGMSG_WARNING,"no hw informations\n");
    process_packet_fcn = process_0mq;

    bank_descr_ptr = getBankDescription(0);
    if(!bank_descr_ptr)
    {
      dbg_printf(DBGMSG_ERROR, "process_0mq_construct(): can not get bank description pointer for bank 0\n");
    ...
    {
      dbg_printf(DBGMSG_ERROR, "process_0mq_construct(): naxis must be 1 or 2 for this filler type\n");
  ...
  else if (nbank > 1)
  {
    ...
    {
      dbg_printf(DBGMSG_ERROR, "process_0mq_construct(): no valid bank mapping array\n");
    ...
    {
      dbg_printf(DBGMSG_ERROR, "process_0mq_construct(): rank of bank mapping array must be 1\n");
      return SINQHM_ERR_NO_BANK_MAPPING_ARRAY;
    }    
    ...
    process_packet_fcn = process_0mq_mb;

    banks = (banks_type *)rt_malloc((nbank * sizeof(banks_type)));
    if(!banks)
    {
      dbg_printf(DBGMSG_ERROR, "process_0mq_construct(): can not allocate memory for banks array\n");
    ...
    for (bank=0; bank<1; bank++)
    {
      ...
      if(!bank_descr_ptr)
      {
        dbg_printf(DBGMSG_ERROR, "process_0mq_construct(): can not get bank description pointer for bank %d\n",bank);
        process_0mq_destruct();
        return SINQHM_ERR_NO_BANK_DESCR_PTR;
      }
      ...
      if(!banks[bank].data)
      {
        dbg_printf(DBGMSG_ERROR, "process_0mq_construct(): can not get bank data pointer\n");
      ...
      if(!banks[bank].axis)
      {
        dbg_printf(DBGMSG_ERROR, "process_0mq_construct(): can not allocate memory for axis array\n");
        return -108;
      }   
...
void process_0mq(packet_type *p)
{
  ...
  UINT32_INC_CEIL_CNT_OVL(histoDataPtr[p->data[2]], shm_cfg_ptr[CFG_FIL_BIN_OVERFLOWS]);



  /* header_type = p->data[0] & LWL_HDR_TYPE_MASK; */

  /* if ((header_type >= LWL_0MQ_C1) && (header_type <= LWL_0MQ_C9)) */
  /* { */
  ...
  /*         zeromqpos = axis_1.fcn(timestamp, &axis_1); */
  /*         if (zeromqpos>=0) */
  /*         {  */
  /*           dbg_printf(DBGMSG_INFO7,"channel=%d zeromqpos=%d\n",channel,zeromqpos); */
  /*           binpos = (channel * axis_1.len) + zeromqpos; */
...
void process_0mq_mb(packet_type *p)
{
  ...
  /*         timestamp = p->data[0] & LWL_HDR_TS_MASK; */
  /*         dbg_printf(DBGMSG_INFO7,"timestamp=%d --",timestamp); */
  /*         zeromqpos = banks[bank].axis[1].fcn(timestamp, &banks[bank].axis[1]); */
  /*         if (zeromqpos>=0) */
  /*         {  */
  /*           dbg_printf(DBGMSG_INFO7,"channel=%d zeromqpos=%d\n",channel,zeromqpos); */
  /*           binpos = (channel * banks[bank].axis[1].len) + zeromqpos; */
