...
void rotateLog()
{
  ...
  if (logRotationCount)
  {
   printed_len = snprintf(buffer, sizeof(buffer), "Log rotation count: %u\n", logRotationCount);
...
//width=\"160\" height=\"34\"
  ...
  sprintf(buffer,"<td> LWL Packets read: %u </td>",shm_cfg_ptr[CFG_FIL_PKG_READ]);
...
static void formatAxis(MaRequest *rq,int bank, int axis)
{
  ...
  switch(mapping)
  {
    ...
    case AXCALC:
      ...
      break;
    case AXBOUNDARY:
        maWriteFmt(rq,"<tr><th>threshold</th><td>%d</td></tr>\r\n",
          axis_descr_ptr->threshold);
        maWriteFmt(rq,"<tr><th>offset</th><td>%d</td></tr>\r\n",
          axis_descr_ptr->offset);
    case AXLOOKUP:
      rank = axisData_ptr[0];
...
static void printDaqStatus(MaRequest *rq)
{
  char buffer[256];  
  volatile axis_descr_type  *axis_descr_ptr;
  int axis;
  ...
  maWriteFmt(rq,"Events processed by filler       : %10u\n",shm_cfg_ptr[CFG_FIL_EVT_PROCESSED]);
  maWriteFmt(rq,"Events skipped due to mask       : %10u\n",shm_cfg_ptr[CFG_FIL_EVT_SKIPPED]);
  maWriteFmt(rq,"Events unknown                   : %10u\n",shm_cfg_ptr[CFG_FIL_PKG_UNKNOWN]);
  maWriteStr(rq,"\n");
  ...
  for (bank=0; bank<histo_descr_ptr->nBank; bank++)
  {
    ...
    do
    {
      ...
      if (axis_descr_ptr)
      {
        ...
        maWriteFmt(rq,"  counts low:  %10u",axis_descr_ptr->cnt_low);
        maWriteFmt(rq,"  counts high: %10u\n",axis_descr_ptr->cnt_high);
      }
    }
  ...
  maWriteStr(rq,"\n");
  maWriteFmt(rq,"Packets read                     : %10u\n",shm_cfg_ptr[CFG_FIL_PKG_READ]);
  maWriteFmt(rq,"Packets incomplete               : %10u\n",shm_cfg_ptr[CFG_FIL_PKG_INCOMPLETE]);
  maWriteFmt(rq,"Packets oversized                : %10u\n",shm_cfg_ptr[CFG_FIL_PKG_OVERSIZE]);
  maWriteStr(rq,"\n");
  maWriteFmt(rq,"TSI Packets (total)              : %10u\n",shm_cfg_ptr[CFG_FIL_TSI_COUNT]);
  maWriteFmt(rq,"TSI Packets with DAQ started     : %10u\n",shm_cfg_ptr[CFG_FIL_TSI_DAQ_STARTED]);
  maWriteFmt(rq,"TSI Packets with DAQ paused      : %10u\n",shm_cfg_ptr[CFG_FIL_TSI_DAQ_PAUSED]);
  maWriteFmt(rq,"TSI Packets with DAQ stopped     : %10u\n",shm_cfg_ptr[CFG_FIL_TSI_DAQ_STOPPED]);
  maWriteStr(rq,"\n");
  maWriteFmt(rq,"TSI Packets with DAQ active Mask : %10u\n",shm_cfg_ptr[CFG_FIL_TSI_DAQ_ACTIVE]);
  maWriteFmt(rq,"TSI Packets with SYNC0           : %10u\n",shm_cfg_ptr[CFG_FIL_TSI_DAQ_SYNC0]);
  maWriteFmt(rq,"TSI Packets with SYNC1           : %10u\n",shm_cfg_ptr[CFG_FIL_TSI_DAQ_SYNC1]);
  maWriteFmt(rq,"TSI Packets with SYNC2           : %10u\n",shm_cfg_ptr[CFG_FIL_TSI_DAQ_SYNC2]);
  maWriteFmt(rq,"TSI Packets with SYNC3           : %10u\n",shm_cfg_ptr[CFG_FIL_TSI_DAQ_SYNC3]);
  maWriteStr(rq,"\n");
  maWriteFmt(rq,"PSD Detector Hits (from TDC)     : %10u\n",shm_cfg_ptr[CFG_FIL_PSD_HITS]);
  maWriteFmt(rq,"PSD Detector Flashes (from TDC)  : %10u\n",shm_cfg_ptr[CFG_FIL_PSD_FLASH]);
  maWriteStr(rq,"\n");
...
//  maWriteFmt(rq,"TSI Packets with Power Fail     : %10d\n",shm_cfg_ptr[CFG_FIL_TSI_POWER_FAIL]);
  maWriteFmt(rq,"TSI Packets with SWC changed     : %10u\n",shm_cfg_ptr[CFG_FIL_TSI_STATUS_WORD_CHANGED]);
  maWriteStr(rq,"\n");
  maWriteFmt(rq,"Detected Fifo Overflows          : %10u\n",shm_cfg_ptr[CFG_FIL_CNT_FIFO_OVERFLOW]);
  maWriteFmt(rq,"Detected Taxi Chip Erros         : %10u\n",shm_cfg_ptr[CFG_FIL_CNT_TAXI_ERROR]);
...
static void daqStatusEgi(MaRequest *rq, char *script, char *uri, char *query,
char *postData, int postLen)
{
  requestLog(rq, uri);
...
/******************************************************************************/
  maWriteFmt(rq,"<h2>Current Acquisition</h2>\n");
/******************************************************************************/
  printDaqStatus(rq);

/******************************************************************************/
  maWriteFmt(rq,"<h2>Current LWL Status</h2>\n");
/******************************************************************************/
...
/******************************************************************************/
  ...
  maWriteFmt(rq,"Web Server Version :  %s %s-%s %s\n", BLD_PRODUCT, BLD_VERSION, BLD_NUMBER, BLD_TYPE);
  maWriteFmt(rq,"Web Server Build   :  %s %s\n\n",__DATE__,__TIME__  );
...
char *postData, int postLen)
{
  ...
  maWriteFmt(rq,"LWL-Overflow-Count: %u\n",val);
...
//  maDefineEgiForm("/index.egi",                indexEgi);
  ...
  maDefineEgiForm("/admin/daqstatus.egi",      daqStatusEgi);
